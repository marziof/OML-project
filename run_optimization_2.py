...
─────────────────────
Refined grid search over alpha, alpha_pull, and eta for EASGD2 across ALL
benchmark functions. Search ranges narrowed from heatmap analysis:

  eta        : [5e-4, 2.3e-3]  — log-spaced (high eta ≥ 0.01 diverges)
  alpha      : [0.325, 1.0]    — linear     (alpha ≤ 0.1 is consistently bad)
  alpha_pull : [0.55, 0.775]   — linear     (outside this band loses ~10×)

Locked hyperparameters (updated from heatmap evidence):
  num_workers = 4    (3–4 workers optimal; 1–2 unstable at any eta)
  beta        = 2.0  (beta 2–5 best; higher beta diverges at eta > 0.002)
  num_epochs  = 2000
...
# Quick smoke-test (3×3×3)
python run_optimization_2.py --quick

# Override locked params if needed
python run_optimization_2.py --num_workers 3 --beta 5 --num_epochs 3000
...
# ── Search grids ──────────────────────────────────────────────────────────────
# Refined ranges based on heatmap analysis:
#
#   eta:        was [1e-3, 1e-1] (5 log vals)  ->  [5e-4, 2.5e-3] (6 log vals)
#               upper bound cut: eta >= 0.01 diverges across all beta/worker counts
#
#   alpha:      was [0.1, 1.0]  (5 linear vals) ->  [0.325, 1.0] (6 linear vals)
#               lower bound cut: alpha=0.1 is the worst cell in every row
#
#   alpha_pull: was [0.1, 1.0]  (5 linear vals) ->  [0.45, 0.85] (5 linear vals)
#               centred on the [0.55, 0.775] sweet spot; extreme values dropped

ETA_GRID        = [round(v, 7) for v in np.logspace(-3.3, -2.6, 6).tolist()]  # ~5e-4 -> ~2.5e-3
ALPHA_GRID      = [round(0.325 + i * 0.135, 4) for i in range(6)]             # 0.325 -> 1.0
ALPHA_PULL_GRID = [round(0.45  + i * 0.10,  4) for i in range(5)]             # 0.45  -> 0.85
...
FUNCTIONS = {
    "sphere":          (sphere_fn,     sphere_gd,     [5.0,  5.0],   2.0),
    ...
    "styblinski_tang": (styblinski_tang_fn, styblinski_tang_gd, [4.0,  4.0],   4.0),
    "ackley":          (ackley_fn,     ackley_gd,     [3.0,  3.0],   4.0),
}
...
def run_grid(locked, eta_grid, alpha_grid, alpha_pull_grid, csv_file, figures_dir):
    os.makedirs(figures_dir, exist_ok=True)

    done = load_done(csv_file)
    combos = list(itertools.product(
        FUNCTIONS.keys(), eta_grid, alpha_grid, alpha_pull_grid
    ))
    total   = len(combos)
    ...
    print(f"eta × alpha × alpha_pull × functions = "
          f"{len(eta_grid)} × {len(alpha_grid)} × {len(alpha_pull_grid)} × {len(FUNCTIONS)}"
          f" = {total} runs  ({skipped} already done, resuming)\n")

    current = 0
    for fn_name, eta, alpha, alpha_pull in combos:
        current += 1
        key = (fn_name, eta, alpha, alpha_pull)

        if key in done:
            print(f"  [{current:>4}/{total}] SKIP  {fn_name}  eta={eta}  alpha={alpha}  alpha_pull={alpha_pull}")
            continue
        ...
        print(f"  [{current:>4}/{total}]  {fn_name:<16}  eta={eta:<8}  "
...
def print_summary(csv_file, top_n=3):
    import csv as _csv
    ...
    for fn_name in sorted(by_fn):
        ...
        for r in fn_rows[:top_n]:
            print(f"  {r['eta']:<10} {r['alpha']:<8} {r['alpha_pull']:<12} "
                  f"{float(r['final_loss']):<14.4e} {float(r['best_loss']):<14.4e} "
                  f"{'YES' if r['converged']=='1' else ' no'}")

    print("\n" + "═"*70)
    print("  GLOBAL TOP CONFIGS  (across all functions, by final_loss)")
    print("═"*70)
    global_best = sorted(rows, key=lambda r: float(r["final_loss"]))[:top_n * len(by_fn)]
    print(f"  {'function':<18} {'eta':<10} {'alpha':<8} {'alpha_pull':<12} {'final_loss'}")
    print(f"  {'-'*16:<18} {'-'*8:<10} {'-'*6:<8} {'-'*10:<12} -----------")
    for r in global_best[:top_n * 2]:
        print(f"  {r['function']:<18} {r['eta']:<10} {r['alpha']:<8} {r['alpha_pull']:<12} "
              f"{float(r['final_loss']):.4e}")
    print()
...
def parse_args():
    ...
    p.add_argument("--beta",        type=float, default=LOCKED["beta"])
...
def main():
    ...
    if args.quick:
        eta_grid        = [0.0005, 0.001, 0.0025]   # within refined safe zone
    ...
    if not args.summary_only:
        run_grid(locked, eta_grid, alpha_grid, alpha_pull_grid,
